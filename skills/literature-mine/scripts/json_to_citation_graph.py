#!/usr/bin/env python3
"""
Convert JSON paper data to an interactive HTML citation graph visualization.

Usage:
    python json_to_citation_graph.py input.json -o output.html
    python json_to_citation_graph.py input.json  # outputs to citation_graph.html
"""
...
<body>
    <div id="container">
        <div id="stats">
            ...
            <div class="stat-item">Papers: <span class="stat-value">{paper_count}</span></div>
        </div>
...
def generate_html(data: dict) -> str:
    """Generate HTML content from paper data."""
    # Calculate statistics
    papers = data.get('data', [])
    paper_count = len(papers)

    # Count internal citations (citations between papers in the dataset)
    citation_count = 0
    paper_ids = {p['id'] for p in papers}
    for paper in papers:
        for cited_id in paper.get('cited', []):
    ...
    # Format data for JSON embedding
...
def main():
    parser = argparse.ArgumentParser(
        description='Convert JSON paper data to interactive HTML citation graph',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog='''
Example:
    python json_to_citation_graph.py papers.json -o graph.html
    python json_to_citation_graph.py papers.json
...
JSON format expected:
    ...
    parser.add_argument('-o', '--output', help='Output HTML file path (default: citation_graph.html)',
                        default='citation_graph.html')
    ...
    print(f"  - Papers: {len(data.get('data', []))}")
